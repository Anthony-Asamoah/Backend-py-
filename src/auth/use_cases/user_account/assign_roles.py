    ...
    if background_tasks and role_ids:
        ...
        try:
            ...
            if user_info:
                ...
                roles_assigned_list = f"<ul>{''.join([f'<li>Role {str(role_id)[:8]}</li>' for role_id in role_ids])}</ul>"

                # Format assignment date
                assignment_date = datetime.now().strftime("%B %d, %Y at %I:%M %p")

                # Send role assignment notification
                await notification_service.create(
                    payload=NotificationCreate(
                        template_name="ROLE-ASSIGNED",
                        context={
                            "user_name": user_name,
                            "platform_name": settings.APP_TITLE,
                            "roles_assigned": roles_assigned_list,
                            "assigned_by": assigned_by_name,
                            "assignment_date": assignment_date,
                            "responsibilities": "You now have access to additional features and capabilities. Please review your new permissions."
                        },
                ...
                log.info(f'Role assignment notification sent to {identifier} via {channel}')

            else:
                log.warning(f'User info not found for account {account.id}')

        except Exception as e:
            log.error(f'Failed to send role assignment notification: {e}')
